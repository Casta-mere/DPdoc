...
def get_device_cpu(device_info):
    ...
    with open(CPU_FILE, 'w') as file:
        file.write("cpu " + str(args.cpu) +" session !!!!!!!!!!!!!!!! 10 times at %s \n" % time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()) )
...
def formatOutput(device_ip, device_info):
    ...
    headers = ["policy_name", "ip", "zone", "session", "speed", "project_id", "user_id"]
...
def data_process(device_ip, device_info):
    ...
    rows = [("FIP_IN_" + i[0], i[0], zone, i[1][0], 0, i[1][1], i[1][2]) for i in trancatedData]
    return rows
...
def get_project_id(resource_id, resource_type):
    if resource_type not in RESOURCE_TYPE:
        return None
    return "select project_id from %s where id = '%s';" % (RESOURCE_TYPE[resource_type], resource_id) 
    
def get_userid_id(device):
    sql = get_project_id(device[DMIP["resource_id"]][:36], device[DMIP["resource_type"]])
    try:
    ...
    sql = "SELECT * FROM project WHERE id = '%s';" % project_id
    try:
        domain_id = keystoneDb.execute_sql(sql)[0][5]
    except Exception as e:
        print(e)
        domain_id = None
    
    sql = "SELECT * FROM user WHERE domain_id = '%s';" % domain_id
    try:
        user_id = keystoneDb.execute_sql(sql)[0][0]
    except Exception as e:
        print(e)
        user_id = None
...
if __name__ == '__main__':
    ...
    neutronUrl, keystoneUrl = readDB(NEUTRON_CONFIG)
    neutronDb = MySQL(neutronUrl)
    keystoneDb = MySQL(keystoneUrl)
    ...
    for i in devices:
        try:
            ...
            for i in device_session_ips:
                project_id, user_id = "\\", "\\"
                if i in dummy_ips:
                    project_id, user_id = get_userid_id(i)
                device_info[i].append(project_id)
                device_info[i].append(user_id)
